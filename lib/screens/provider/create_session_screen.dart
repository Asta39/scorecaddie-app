...
class _CreateSessionScreenState extends ConsumerState<CreateSessionScreen> {
  ...
  String _sessionType = 'Group';
  String _locationArea = 'Driving Range';
  String _targetSkillLevel = 'All';
  ...
  Future<void> _createSession() async {
    ...
    try {
      ...
      // Invalidate the session lists so the UI reflects the newly created session
      ref.invalidate(coachSessionsProvider);
      
      // Also specifically invalidate the family provider for this coach
      final currentUser = ref.read(authStateProvider).valueOrNull;
      if (currentUser != null) {
        ref.invalidate(providerSessionsProvider(currentUser.id));
      }


      if (mounted) {
        setState(() => _isLoading = false);
        TopNotification.showSuccess(context, 'Session Published Successfully!');
        context.pop();
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
        TopNotification.showError(context, 'Error creating session: $e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Create Session', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 18)),
        elevation: 0,
        ...
        actions: [
          TextButton(
            ...
            child: _isLoading 
              ...
              : const Text('Publish', style: TextStyle(fontWeight: FontWeight.w800, color: AppColors.purple700)),
          ),
        ],
      ),
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: _inputDecoration('Prerequisites (e.g. Bring your own sticks)', LucideIcons.checkSquare),
              maxLines: 2,
            ),
            const SizedBox(height: 16),
